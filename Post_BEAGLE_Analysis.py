...
class Beagle_Output(dict):
    ...
    def convolve_wrapper(self, args):
        
        r_wave, r_curve, model_wave, model_flux, wave_obs_spectra, oversample = args
        
        return self.convolve_model_spec(r_wave, r_curve, model_wave, model_flux, wave_obs_spectra, oversample)

    def test_generate_convolved_spectra(self, r_wave, r_curve, obs_wave, oversample):
        ...
        data = np.arange(0, len(in_range_flux), 1)

        column_1pz_vector = (1 + np.array(self['posterior']['redshift'].astype(float))).reshape(-1, 1)

        obs_wave_model = self['full_sed_wvln'].astype(float) * column_1pz_vector

        
        full_sed_input = [(np.array(r_wave), np.array(r_curve), model_wav, model_flux, obs_wave, oversample) \
                          for model_wav, model_flux in zip(obs_wave_model, in_range_flux)]

        full_agn_input = [(np.array(r_wave), np.array(r_curve), model_wav, model_flux, obs_wave, oversample) \
                          for model_wav, model_flux in zip(obs_wave_model, in_range_agn)]

        rand_idx = np.random.randint(0, len(in_range_flux), 1)[0]

        
        #print(rand_idx)
                                    
        #print(full_sed_input[rand_idx])
        #print(full_agn_input[rand_idx])

        #return full_sed_input
        
        with Pool(processes=4) as pool: 
        
            #results = pool.map(self.convolve_wrapper, full_sed_input)
            results = list(tqdm(pool.imap(self.convolve_wrapper, full_sed_input), total=len(full_sed_input)))
            #full_agn_convolved = pool.map(self.convolve_wrapper, full_agn_input)

        return results
        #self['convolved_spectra']   = full_sed_convolved
        #self['convolved_agn']       = full_agn_convolved
    ...
    def generate_convolved_spectra(self, r_wave, r_curve, obs_wave, oversample):
        ...
        convolved_agn = np.zeros((len(in_range_flux), len(obs_wave)))
        data = np.arange(0, len(in_range_flux), 1)
        for i in tqdm(data, desc="Processing", ncols=70):
            #print(f'Convolving Spectra {i+1} of {len(in_range_flux)}')
            
            z = redshift[i]
            model_wav = self['full_sed_wvln'] * (1+z) #converts to observed frame
    ...
    def generate_corner_plot(self, params = None):

        fig = plt.figure(figsize = (20, 15))
        
        if params is not None:
            ax = sb.pairplot(self['posterior'][params].to_pandas(), 
                             plot_kws = {'marker': ',', 
                                         's': 1, 
                                         'color': 'purple'},
                             diag_kws = {'color': 'purple', 
                                         'kde':True},
                             corner = True)
            ax.map_lower(sb.kdeplot, levels=[0.16, 0.5, 0.84],color = 'black', zorder = 9, alpha = 0.75)
        
        else:
            
            ax = sb.pairplot(self['posterior'][ 'mass',
                                                 'agn_lacc',
                                                 'nebular_logu',
                                                 'nebular_z',
                                                 'tauv_eff',
                                                 ...
                                                 'metallicity',
                                                 'specific_sfr',
                                                 'current_sfr_timescale'].to_pandas(), 
                        plot_kws = {'marker': ',', 
                                    's': 1, 
                                    'color': 'purple'},
                        diag_kws = {'color': 'purple', 
                                   'kde':True},
                        corner = True)
            ax.map_lower(sb.kdeplot, levels=[0.025, 0.16, 0.5, 0.84, 0.975],color = 'black', zorder = 9, alpha = 0.75)
